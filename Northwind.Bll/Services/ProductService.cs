    ...
    public class ProductService : BaseService, IProductService
    {
        ...
        public async Task<IEnumerable<Product>> GetProductsAsync(CancellationToken cancellationToken, int amount = 0)
        {
            var products = amount == 0
                ...
                { nameof(Product.Supplier), nameof(Product.Category) }).OrderBy(p => p.ProductId).ToArrayAsync()
                : await _productRepository.Queryable().AsNoTracking()
                .OrderBy(p => p.ProductId).Take(amount).Include(p => p.Supplier).Include(p => p.Category).ToArrayAsync();
